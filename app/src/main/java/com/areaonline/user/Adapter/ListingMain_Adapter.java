...
import java.util.HashMap;
    ...
    public class ListingMain_Adapter extends RecyclerView.Adapter<com.areaonline.user.Adapter.ListingMain_Adapter.ViewHolder> {
        ...
        public ListingMain_Adapter(Activity activity, ArrayList<HashMap<String, String>>  modal) {
            this.response = modal;
        ...
        @NonNull
        @Override
        public com.areaonline.user.Adapter.ListingMain_Adapter.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
            View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.item_newdesign, parent, false);
            return new com.areaonline.user.Adapter.ListingMain_Adapter.ViewHolder(view);
        }

        @RequiresApi(api = Build.VERSION_CODES.LOLLIPOP)
        @SuppressLint("UseCompatLoadingForDrawables")
        @Override
        public void onBindViewHolder(@NonNull com.areaonline.user.Adapter.ListingMain_Adapter.ViewHolder holder, int position) {
            setdata(holder,position);
        }

        @SuppressLint("UseCompatLoadingForDrawables")
        private void setdata(com.areaonline.user.Adapter.ListingMain_Adapter.ViewHolder holder, int position) {
            if (response.get(position).get("listingimg") == null || response.get(position).get("listingimg").equalsIgnoreCase("")  ){
                Glide.with(activity)
                        .asBitmap()
                        .load(activity.getResources().getDrawable(R.drawable.logobw)).placeholder(R.drawable.logobw).diskCacheStrategy(DiskCacheStrategy.NONE).skipMemoryCache(true)
                ...
                Glide.with(activity)
                        ...
                        .skipMemoryCache(true).error((Glide.with(activity).load("https://www.areaonline.in/uploads/SubCatLogo/"+response.get(position).get("listingimg")))).into(new CustomTarget<Bitmap>() {
                    @Override
                    public void onResourceReady(@NonNull Bitmap resource, @Nullable Transition<? super Bitmap> transition) {
                        holder.iv_listing.setImageBitmap(resource);
                    }
...
//            if (loadfailed){
//                Glide.with(activity).load("https://www.areaonline.in/uploads/SubCatLogo/"+response.get(position).get("listingimg").toString()).diskCacheStrategy(DiskCacheStrategy.NONE)
